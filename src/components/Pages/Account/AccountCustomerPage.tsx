...
/* Custom packages */
import './AccountCustomerPage.css';
import { generateIdFunc } from '../../Tools/methodForest';
import { language } from '../../Tools/language';
import { refIdType } from '../../Tools/type';
import { _defaultLanguage_ } from '../../Tools/constants';
import AccountCustomerFeedWidget from '../../Widgets/Account/Customer/AccountCustomerFeedWidget';
...
const AccountAdminPage = (props: propsType, ref: any) => {
    ...
    const showFunc = (x: { show: boolean }) => {
        $('#acup_scaffold').css({ 'z-index': x.show ? 5 : 1 });
        !x.show && $('#acup_scaffold').removeAttr('style');
    };
    ...
    if (callCenterFeed.length === 0) for (let i = 0; i < testFeed.length; i++) callCenterFeed.push(<AccountCustomerFeedWidget key={i} ref={feedRef[i]} $data={{ refId: feedRef[i], controllerRef: { current: undefined }, rootControllers: rootControllers, accountData: testFeed[i] }} />);
    /* - */
    const component = <>
        <div id='acup_scaffold' className='prevent_select'>
            <div id='acup_header'>
                <div id='acup_header_glass' className='glass'></div>
                <div id='acup_header_container'>
                    <div id='acup_header_title_container'>
                        <h1 id='acup_title'>{traduction['t0010']}</h1>
                    ...
                    <div id='acup_table_header'>
                        <div className='accfw_checkbox_container'>
                            <input id='aap_checkbox' type='checkbox' />
                        </div>
                        <div className='accfw_hyphen' />

                        <p className='acufw_full_name one_line'>Name</p>
                        <div className='accfw_hyphen' />

                        <p className='acufw_email one_line'>Email</p>
                        <div className='accfw_hyphen' />

                        <p className='acufw_phone one_line'>Phone</p>
                        <div className='accfw_hyphen' />

                        <p className='acufw_expiration one_line'>Expire dans</p>
                        <div className='accfw_hyphen' />
